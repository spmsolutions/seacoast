...
export function IntegrationFilters({
  ...
  return (
    <>
      {onlyUse.includes("dataFilter") && <DataFilter filter={filter} />}

      {onlyUse.includes("orderFilter") && <OrderFilter filter={filter} />}
...
export function AdditionalContentFilter({ filter }) {
  return (
    <>
      <p>
        Allows adding custom content before the field element's closing tag.
        Useful for adding markup, styles, etc.
      </p>
...
function DataFilter({ filter }) {
  return (
    <>
      <h2>Data filter</h2>
      <p>
        Allows modifying form field data before it's shown in the Block Editor
        or output on the frontend.
      </p>
      <p>
        Useful if, for example, you want to ensure that all fields fit into a
        2-column layout.
      </p>
...
function OrderFilter({ filter }) {
  return (
    <>
      <h2>Order filter</h2>
      <p>
        Forces a form field order, regardless of how it's set in the Block
        editor. Fields not modified through the filter will use order in which
        they're set in the Block editor.
      </p>
      <p>
        Not all fields need to have an order defined. For example, you may want
        to make sure <code>firstname</code>, <code>lastname</code> and{" "}
        <code>email</code> are displayed first, but other fields follow their
        Block editor order.
      </p>
...
function PrePostIdFilter({ filter }) {
  return (
    <>
      ...
      <p>
        Allows updating item IDs sent to external integrations to which the form
        data is sent.
      </p>
...
function PrePostParamsFilter({ filter }) {
  ...
  return (
    <>
      ...
      <p>
        Allows modifying form field data before it's sent to {text}. Useful if
        you want to make values derived from the sent data, or add new fields.
      </p>
